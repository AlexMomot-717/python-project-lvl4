...
def test_create_status_with_existing_name_post(client: Client) -> None:
    # given
    service_user = create_service_user()
    client.force_login(service_user)
    create_status(name="active")
    route = "/statuses/create/"
    form_data = {
        "name": "active",
    }
    ...
    assert "create_status.html" in [temp.name for temp in response.templates]
